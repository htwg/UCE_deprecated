...
/**
 * Used to export remote objects using a {@link RelaySocketFactory} both as
...
public final class RelayRemoteObject extends UnicastRemoteObject {
     ...
     */
    private RelayRemoteObject(int port, RelaySocketFactory fac) throws RemoteException {
        super(port, fac, fac);
    }
    ...
    protected RelayRemoteObject(InetSocketAddress relayServerEndpoint) throws RemoteException {
        this(0, new RelaySocketFactory(relayServerEndpoint));
    }
    ...
    protected RelayRemoteObject(int port, InetSocketAddress relayServerEndpoint)
            throws RemoteException {
        this(port, new RelaySocketFactory(relayServerEndpoint));
    }
    ...
    public static Remote exportObject(Remote obj, int port, InetSocketAddress relayServerEndpoint)
            throws RemoteException {
        RelaySocketFactory fac = new RelaySocketFactory(relayServerEndpoint);
